 ...
 * @copyright
 **********************************************************************************************************************/
/** @defgroup qspi_group QSPI
 *  @ingroup  peripheral_group
 *  @{
 *  @brief  qspi definitions, structures, and functions
*/
...
#define qspi_init(qspi_id, spi_mode)                                     Qspi_Init(qspi_id, spi_mode)
#if defined(CONFIG_SUPPORT_QSPI0_MULTI_CS)
#define qspi0_set_device_polarity(slave_id, slave_polarity)              #define Qspi0_Set_Device_Polarity(slave_id, slave_polarity)
#endif
#define qspi_transfer(qspi_id, qspi_block_request)                        Qspi_Transfer(qspi_id, qspi_block_request)
#define qspi_change_clk(qspi_id, spi_clk)                                 Qspi_Change_Clk(qspi_id, spi_clk)
#define qspi_write_dma(qspi_id,qspi_block_request,finish_proc_cb)         Qspi_Write_Dma(qspi_id,qspi_block_request,finish_proc_cb)
#define qspi_read_dma(qspi_id,qspi_block_request,finish_proc_cb)          Qspi_Read_Dma(qspi_id,qspi_block_request,finish_proc_cb)
#define spi_transfer(qspi_id,qspi_block_request,finish_proc_cb)           Spi_Transfer(qspi_id,qspi_block_request,finish_proc_cb)
#define spi_transfer_pio(qspi_id,qspi_block_request)                      Spi_Transfer_Pio(qspi_id,qspi_block_request)
#define get_qspi_state(qspi_id)                                           Get_Qspi_State(qspi_id)
#define spi_transfer_slave_abort(qspi_id)                                 Spi_Transfer_Slave_Abort(qspi_id)
#define qspi_FIFO_clear(qspi_id)                                          Qspi_FIFO_Clear(qspi_id)
/**
...
typedef struct
{
    ...
    uint8_t   pre_delay_count;
    uint8_t   inter_delay_count;
    uint8_t   pose_delay_count;
    uint8_t   reserved3;
...
/*@}*/ /* end of peripheral_group QSPI_Drver */
