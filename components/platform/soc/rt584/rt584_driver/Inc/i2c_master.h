 ...
 *  @{
 *  @brief  I2C_Master_Driver header information
*/
#ifndef __RT584_I2C_MASTER_H__
#define __RT584_I2C_MASTER_H__
 ...
 *************************************************************************************************/
/**
 *  @Brief I2C Master clock definitions
 */
...
#define I2C_CLOCK_1M        4       /*!< I2C Master running in 800KHz */
#define I2C_CLOCK_MAX       5       /*!< I2C Master running in 100KHz */
...
/**
 *  @Brief I2C Master FIFO number definitions
 */
 ...
 *
 */
#define i2c_init(I2C_SPEED)     I2c_Master_Init(0, I2C_SPEED)
 ...
 *
 */
uint32_t i2c_read(const i2c_slave_data_t *slave, uint8_t  *data, uint32_t len, i2cm_proc_cb_t endproc_cb);
/**
...
/**
 * @brief I2C master finish routine callback for user application.
 *
 * @param[in] statue I2C transfer status.
 *
 * @details    This callback function is still running in interrupt mode, so this function
 ...
 *
 */
typedef void (* i2cm_proc_cb)(uint32_t status);
...
typedef struct
{
    i2cm_proc_cb    endproc_cb;         /*!< I2C complete callback function */
    uint8_t         dev_addr;           /*!< I2C device address, 7bits only */
    uint8_t         bFlag_16bits;       /*!< 1 for register address is 16bits, 0 for register address is 8bits. */
    uint16_t        reg_addr;           /*!< I2C register address value */
} i2c_slave_data_ex_t;
 ...
 *
 * @param[in]  master_id   Specifies the I2C number.
 * @param[in]  slave       Specifies the I2C slave address and callback.
 * @param[in]  data        Pointer to buffer with data to write to I2C slave.
 * @param[in]  len         Number of data bytes to transmit, maxmum size is 1024 bytes.
 *
 * @retval
...
#endif      /* end of __RT584_I2C_MASTER_H__ */
