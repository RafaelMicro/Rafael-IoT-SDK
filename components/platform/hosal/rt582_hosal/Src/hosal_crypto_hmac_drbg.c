...
int hosal_crypto_hmac_drbg_operation(volatile hosal_hmac_drbg_dev_t* hmac_drbg)
{
	...
	if(hmac_drbg->crypto_operation==HOSAL_HMAC_DRBG_HMAC)
	{
		...
		hmac_drbg_instantiate(&hmac_drbg_ctx, hmac_drbg->seed_material, hmac_drbg->seed_material_length);
		...
		if(hmac_drbg->data_len==0)
		{
			...
			{
				hmac_drbg_generate(hmac_drbg->out_ptr, &hmac_drbg_ctx, hmac_drbg->out_len, NULL, hmac_drbg->data_len);
			}		
		}
		else
		{  
			...
			{		
			  ...
			  hmac_drbg_generate(hmac_drbg->out_ptr, &hmac_drbg_ctx, hmac_drbg->out_len, (uint8_t*)(hmac_drbg->data_ptr+index), hmac_drbg->data_len);
...
int hosal_hmac_drbg_generate_k(hosal_hmac_drbg_dev_t* hmac_drbg)
{
    ...
    hmac_drbg_instantiate(&hmac_drbg_ctx, hmac_drbg->seed_material, hmac_drbg->seed_material_length);       /*RFC6979 section 3.2 step d~ step g*/

    while (1)
    {
		
        hmac_drbg_generate(hmac_drbg->out_ptr, &hmac_drbg_ctx, hmac_drbg->out_len, NULL, 0); 
...
{
	hosal_hmac_drbg_instantiate(state,seed_material,seed_material_length);
}
