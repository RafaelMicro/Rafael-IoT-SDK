...
static uint32_t continue_ff_addr(uint32_t start, uint32_t end) {
    ...
    for (; start < end; start += sizeof(buf)) {
        read_size = (sizeof(buf) < end - start) ? sizeof(buf) : end - start;
    ...
    return (addr == start) ? end : EFD_WG_ALIGN(addr);
}
...
static uint32_t get_next_env_addr(sector_meta_data_t sector,
    ...
    if (pre_env->addr.start <= sector->addr + SECTOR_SIZE) {
        ...
        addr = (pre_env->crc_is_ok) ? (pre_env->addr.start + pre_env->len)
                                    : (pre_env->addr.start + EFD_WG_ALIGN(1));
        /* check and find next ENV address */
        addr = find_next_env_addr(
            addr, sector->addr + SECTOR_SIZE - SECTOR_HDR_DATA_SIZE);

        return (addr > sector->addr + SECTOR_SIZE || pre_env->len == 0)
                   ? FAILED_ADDR
                   : addr;
    }
    return FAILED_ADDR;
...
static EfErrCode read_env(env_node_obj_t env) {
    ...
    for (len = 0, size = 0; len < crc_data_len; len += size) {
        size = (len + sizeof(buf) < crc_data_len) ? sizeof(buf)
                                                  : (crc_data_len - len);
...
static uint32_t get_next_sector_addr(sector_meta_data_t pre_sec) {
    ...
    uint32_t next_addr = (pre_sec->combined == SECTOR_NOT_COMBINED)
                             ? pre_sec->addr + SECTOR_SIZE
                             : pre_sec->addr + pre_sec->combined * SECTOR_SIZE;
    /* check range */
    return (next_addr < env_start_addr + ENV_AREA_SIZE) ? next_addr
                                                        : FAILED_ADDR;
}
...
static size_t get_env(const char* key, void* value_buf, size_t buf_len,
    ...
    if (find_env(key, &env)) {
        ...
        read_len = (env.value_len < buf_len) ? env.value_len : buf_len;
        if (value_buf)
...
static EfErrCode write_env_hdr(uint32_t addr, env_hdr_data_t env_hdr) {
    ...
    EfErrCode result = write_status(addr, env_hdr->status_table, ENV_STATUS_NUM,
                                    ENV_PRE_WRITE);
    if (result != EFD_NO_ERR)
        return result;

    /* write other header data */
    return efd_port_write(addr + ENV_MAGIC_OFFSET, &env_hdr->magic,
...
#endif
    if (old_env->addr.start)
        return EFD_ENV_ARG_ERR;
...
static EfErrCode move_env(env_node_obj_t env) {
    ...
    /* start move the ENV */
    {
        ...
        for (len = 0, size = 0; len < env_len; len += size) {
            size = (len + sizeof(buf) < env_len) ? sizeof(buf)
                                                 : (env_len - len);
...
EfErrCode efd_env_set_default(void) {
    ...
    for (i = 0; i < default_env_set_size; i++) {
        ...
        value_len = (default_env_set[i].value_len == 0)
                        ? strlen(default_env_set[i].value)
                        : default_env_set[i].value_len;
        ...
        if (env->status == ENV_WRITE) {
            ...
            __reload:
                ...
                for (len = 0, size = 0; len < env->value_len; len += size) {
                    size = (env->value_len - len > sizeof(buf))
                               ? sizeof(buf)
                               : env->value_len - len;
