...
#define READ_BUF_SIZE 32
    ...
    while (data_start < sector_start + EFD_ERASE_MIN_SIZE) {
        read_buf_size = (data_start + READ_BUF_SIZE
                         < sector_start + EFD_ERASE_MIN_SIZE)
                            ? READ_BUF_SIZE
                            : sector_start + EFD_ERASE_MIN_SIZE - data_start;

        efd_port_read(data_start, (uint32_t*)buf, read_buf_size);
        for (i = 0; i < read_buf_size; i++)
            continue_ff = (buf[i] == 0xFF) ? continue_ff + 1 : 0;
...
size_t efd_log_get_used_size(void) {
    ...
    physical_size = (log_start_addr < log_end_addr)
                        ? (log_end_addr - log_start_addr)
                        : (LOG_AREA_SIZE - (log_start_addr - log_end_addr));
...
static uint32_t get_next_flash_sec_addr(uint32_t cur_addr) {
    ...
    return (cur_addr % EFD_ERASE_MIN_SIZE == 0)
               ? log_area_start_addr
               : log_area_start_addr + (cur_sec_id + 1) * EFD_ERASE_MIN_SIZE;
}
