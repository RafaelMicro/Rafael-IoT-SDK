...
EfErrCode efd_erase_bak_app(size_t app_size) {
    EfErrCode result = efd_port_erase(efd_get_bak_app_start_addr(), app_size);
    switch (result) {
        case EFD_NO_ERR: {
...
EfErrCode efd_erase_spec_user_app(uint32_t user_app_addr, size_t app_size,
                                  EfErrCode (*app_erase)(uint32_t addr,
                                                         size_t size)) {
    EfErrCode result = app_erase(user_app_addr, app_size);
    switch (result) {
        case EFD_NO_ERR: {
...
EfErrCode efd_erase_bl(uint32_t bl_addr, size_t bl_size) {
    EfErrCode result = efd_port_erase(bl_addr, bl_size);
    switch (result) {
        case EFD_NO_ERR: {
