...
extern ble_err_t (* const Prcss_BLE_OPCODE_ATT[])(void);
extern ble_err_t Prcss_OPCODE_ATT_Write_Command(void);
extern ble_err_t Prcss_OPCODE_ATT_Signed_Write_Command(void);

extern ble_err_t setBLE_ConnTxData(uint16_t connID, uint8_t *ScanRspData, uint8_t Length);
extern ble_err_t ATT_Request_ERROR_RESPONSE(uint16_t connID, uint8_t ReqOpcode_inError, uint16_t AttHandle_inError, uint8_t ErrorCode);
extern ble_err_t ATT_Request(uint16_t connID, uint8_t AttOpcode, uint8_t *paramGroup,  uint16_t dataLength);
