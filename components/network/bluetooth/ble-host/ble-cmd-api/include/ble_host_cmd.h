...
//privacy add
ble_err_t private_address_construct_by_id(uint8_t host_id, private_addr_gen_t *private_addr_param);
ble_err_t private_address_resolve_by_id(uint8_t host_id, uint8_t *private_addr24);
 ...
 * @retval  -1: the queue is full or memory not enough.
 */
ble_err_t bhc_host_prcss_l2cap_cid(uint16_t conn_handle, uint8_t *p_data, uint16_t length);
