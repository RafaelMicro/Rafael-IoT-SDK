...
use_noninlined_mul64 seta 0 ; 1 to enable, 0 to disable (2.6%/4.0% slower (depending on use_mul_for_sqr) if enabled but saves 308 bytes of compiled code size)

	gbla use_noninlined_sqr64
use_noninlined_sqr64 seta 0 ; 1 to enable, 0 to disable (2.4% slower if enabled and use_mul_for_sqr=0 but saves 164 bytes of compiled code size)

	gbla use_interpreter
use_interpreter seta 0 ; 1 to enable, 0 to disable (3.6% slower if enabled but saves 268 bytes of compiled code size)
