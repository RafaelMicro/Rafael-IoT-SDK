...
void Rfb_MacFrameGen(uint8_t modem_type, MacBuffer_t *pbuf, uint8_t *tx_control, uint8_t Dsn, uint16_t MacDataLength)
{
    uint16_t idx;
    uint16_t payloadLength = MacDataLength;
    uint8_t initialCW;
    uint8_t ackRequest;
    uint16_t max_data_size = (modem_type == SUBG_CTRL_MODU_FSK) ? FSK_MAX_DATA_SIZE : OQPSK_MAX_DATA_SIZE;
