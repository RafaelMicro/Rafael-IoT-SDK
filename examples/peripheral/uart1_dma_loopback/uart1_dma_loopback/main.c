...
int main(void) {
    ...
    printf("hosal uart_1 dma polling examples %s %s \r\n", __DATE__, __TIME__);
    ...
    for (i = 1; i < 1024; i++) {
        ...
        while (!hosal_uart_rx_dma_ready(&uart_dev)) {;}
        hosal_uart_rx_dma(&uart_dev, recvbuf, i);

        while (!hosal_uart_tx_dma_ready(&uart_dev)) {;}
        hosal_uart_tx_dma(&uart_dev, sendbuf, i);

        while (!hosal_uart_tx_dma_finish(&uart_dev)) {;}
        while (!hosal_uart_rx_dma_finish(&uart_dev)) {;}
