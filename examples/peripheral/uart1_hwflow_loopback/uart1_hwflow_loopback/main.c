...
int main(void) {
    ...
    printf("hosal uart1 hw flow control examples %s %s \r\n", __DATE__, __TIME__);
    ...
    uart_dev.config.rts_pin = 20;
    uart_dev.config.cts_pin = 21;
    uart_dev.config.tx_pin = 28;
    uart_dev.config.rx_pin = 29;
    uart_dev.config.uart_cfg.uart_id = 1;
    ...
    uart_dev.config.uart_cfg.flow_control = UART_HWFC_ENABLED;
    ...
    for (i = 0; i < 1024; i++) {
        ...
        uart_set_modem_status(uart_dev.config.uart_cfg.uart_id, UART1_HWFL_ENABLE);
        while (!hosal_uart_tx_ready(&uart_dev)) {;}
        hosal_uart_tx_pol(&uart_dev, &sendbuf[i]);

        uart_set_modem_status(uart_dev.config.uart_cfg.uart_id, UART1_HWFL_DISABLE);
