...
void test_pbkdf2_case(uint32_t i, struct pbkdf2_struct*  test_vector) {
    ...
    sha256_dev.crypto_operation = HOSAL_SHA256_PBKDF2_HMAC;
    sha256_dev.pbkdf2 = test_vector;

    result = hosal_crypto_sha256_operation(&sha256_dev);

    if (result != STATUS_SUCCESS) {
        printf("Oops, PBKDF2 case %"PRIu32" input errorr \r\n", i);
        while (1);
    }

    if ( memcmp( (uint8_t*) (test_vector->key_output), (expect_result[i]),
                 test_vector->key_length) != 0) {
        printf("Oops, PBKDF2 case %"PRIu32" errorr \r\n", i);

        while (1);
    }

    printf("PBKDF2 case %"PRIu32" success \r\n", i);
}
...
int main(void) {
    ...
    printf("Build Time:%s \r\n",__TIME__);
    printf("----------------------------------------------------------------\r\n");
    printf("Examples    : hosal crypto PBKDF2 demo \r\n");
    printf("----------------------------------------------------------------\r\n");
    ...
    printf("\r\n\r\n");
    printf("hosal crypto PBKDF2 finish\r\n");
