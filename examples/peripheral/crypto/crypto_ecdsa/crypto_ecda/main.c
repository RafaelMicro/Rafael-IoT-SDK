...
void hmac_drbg_generate_k(uint8_t* msg, uint32_t msg_length, uint8_t*  k) {
    ...
    hmac_drbg_instantiate(&hmac_drbg_ctx, msg,
                          msg_length);       /*RFC6979 section 3.2 step d~ step g*/
...
void  test_ecdsa_signature(uint8_t* msg, uint32_t msg_len, uint8_t* exp_r,
                           uint8_t* exp_s) {
    uint8_t    random_k[32];
    ...
    uint8_t    little_endian_hash[SHA256_DIGEST_SIZE],
               little_endian_k[ECDSA_P256_LENGTH];
    ...
    sha256_dev.in_ptr = msg;
    sha256_dev.in_length = msg_len;
    sha256_dev.out_ptr = h1_hash_value;
    ...
    memcpy(seed_material, private_key,
           ECDSA_P256_LENGTH);             /*because NIST P-256 is 32 bytes*/
    ...
    buffer_endian_exchange( (uint32_t*) little_endian_private,
                            (uint32_t*) private_key, (ECDSA_P256_LENGTH >> 2));
    buffer_endian_exchange( (uint32_t*) little_endian_k, (uint32_t*) random_k,
                            (ECDSA_P256_LENGTH >> 2));
    buffer_endian_exchange( (uint32_t*) little_endian_hash,
                            (uint32_t*) h1_hash_value, (SHA256_DIGEST_SIZE >> 2));
    ...
    ecc_p256.signatrue = &signature;
    ecc_p256.p_hash = (uint32_t*)little_endian_hash;
    ecc_p256.p_key = (uint32_t*)little_endian_private;
    ecc_p256.p_k = (uint32_t*)little_endian_k;
    ...
    buffer_endian_exchange( (uint32_t*) seed_material, (uint32_t*) (signature.r),
                            (ECDSA_P256_LENGTH >> 2));
    buffer_endian_exchange( (uint32_t*) (seed_material + ECDSA_P256_LENGTH),
                            (uint32_t*) (signature.s), (ECDSA_P256_LENGTH >> 2));
...
void test_ecdsa_verify(uint8_t*  msg, uint32_t  msg_length, uint8_t* vector_r,
                       uint8_t* vector_s) {
    sha256_context     sha256_ctx;
    ...
    sha256_dev.crypto_operation = HOSAL_SHA256_DIGEST;
    sha256_dev.in_ptr = msg;
    sha256_dev.in_length = msg_length;
    sha256_dev.out_ptr = hash_value;
    hosal_crypto_sha256_operation(&sha256_dev);
    ...
    buffer_endian_exchange( (uint32_t*) (signature.r), (uint32_t*) vector_r,
                            (ECDSA_P256_LENGTH >> 2));
    buffer_endian_exchange( (uint32_t*) (signature.s), (uint32_t*) vector_s,
                            (ECDSA_P256_LENGTH >> 2));

    buffer_endian_exchange( (uint32_t*) little_endian_hash, (uint32_t*) hash_value,
                            (SHA256_DIGEST_SIZE >> 2));

    buffer_endian_exchange( (uint32_t*) (public_key_LE.x), (uint32_t*) public_key_x,
                            (ECDSA_P256_LENGTH >> 2));
    buffer_endian_exchange( (uint32_t*) (public_key_LE.y), (uint32_t*) public_key_y,
                            (ECDSA_P256_LENGTH >> 2));
...
void test_ecdsa_signature_verify(uint32_t length) {
    ...
    static uint8_t    hash_value[SHA256_DIGEST_SIZE],
           le_hash_value[SHA256_DIGEST_SIZE];
    ...
    if (length <= 16) {
        hosal_flash_read_id(length,device_id);
        //flash_get_unique_id( (uint32_t) device_id,
                             //length);   /*length bytes personal data*/
    } else {
        hosal_flash_read_id( 16,device_id);
                                /*maximum 16 bytes personal data*/
    }
    ...
    sha256_dev.crypto_operation = HOSAL_SHA256_DIGEST;
    sha256_dev.in_ptr = device_id;
    sha256_dev.in_length = length;
    sha256_dev.out_ptr = test_private_key;
    hosal_crypto_sha256_operation(&sha256_dev);
...
#else
    buffer_endian_exchange( (uint32_t*) le_test_private_key,
                            (uint32_t*) test_private_key, (ECDSA_P256_LENGTH >> 2));
#endif
    ...
    ecc_p256.result = &public_key_point_LE;
    ecc_p256.base = (ECPoint_P256*) &Curve_Gx_p256;
    ecc_p256.p_key = (uint32_t*) le_test_private_key;
...

    {
        /*init test pattern*/
        for (i = 3; i < 1024; i++) {
            temp += (test_message[i - 3] * 257) + (test_message[i - 2] * 617) +
                    (test_message[i - 1] * 947);
        ...
        for (j = 0; j < 4096; j++) {
            ...
            for (i = j; i < 4096; i++) {
                ...
                sha256_dev.crypto_operation = HOSAL_SHA256_DIGEST;
                sha256_dev.in_ptr = test_ptr;
                sha256_dev.in_length = count;
                sha256_dev.out_ptr = hash_value;
                hosal_crypto_sha256_operation(&sha256_dev);

                be_print_hex_bytes("hash_value:",  hash_value,  SHA256_DIGEST_SIZE);

                memcpy(seed_material, test_private_key,
                       ECDSA_P256_LENGTH);             /*because NIST P-256 is 32 bytes*/
                ...
                convert_endian(le_random_k, random_k,
                               ECDSA_P256_LENGTH);       /*generate little endian format random key*/
#else
                buffer_endian_exchange( (uint32_t*) le_hash_value, (uint32_t*) hash_value,
                                        (ECDSA_P256_LENGTH >> 2));
                /*in fact, we don't care random_k is little endian or big endian... because it is just a random number... */
                buffer_endian_exchange( (uint32_t*) le_random_k, (uint32_t*) random_k,
                                        (ECDSA_P256_LENGTH >> 2));
#endif
                ...
                ecc_p256.crypto_operation = HOSAL_ECDA_P256_SIGNATURE;
                ecc_p256.signatrue =  &caculate_signature_LE;
                ecc_p256.p_hash = (uint32_t*) le_hash_value;
                ecc_p256.p_key =  (uint32_t*) le_test_private_key;
                ecc_p256.p_k = (uint32_t*) le_random_k;

                hosal_crypto_ecc_p256(&ecc_p256);


                le_print_hex_bytes("r:", (uint8_t*) & (caculate_signature_LE.r),
                                   ECDSA_P256_LENGTH);
                le_print_hex_bytes("s:", (uint8_t*) & (caculate_signature_LE.s),
                                   ECDSA_P256_LENGTH);
                ...
                ecc_p256.signatrue =  &caculate_signature_LE;
                ecc_p256.p_hash = (uint32_t*) le_hash_value;
                ecc_p256.base = &public_key_point_LE;
                ...
                if (status != STATUS_SUCCESS) {
                    ...
                    le_print_hex_bytes("correct r:",  (uint8_t*) & (caculate_signature_LE.r),
                                       ECDSA_P256_LENGTH);
...
int main(void) {
    ...
    printf("Build Time:%s \r\n",__TIME__);
    printf("----------------------------------------------------------------\r\n");
    printf("Examples    : hosal crypto ecda demo \r\n");
    printf("----------------------------------------------------------------\r\n");
    ...
    test_ecdsa_signature((uint8_t*)  message,  6, (uint8_t*) sample_expect_vector_r,
                         (uint8_t*) sample_expect_vector_s);
    test_ecdsa_signature((uint8_t*) message2, 4, (uint8_t*) test_expect_vector_r,
                         (uint8_t*) test_expect_vector_s);

    test_ecdsa_verify((uint8_t*)  message,  6, (uint8_t*) sample_expect_vector_r,
                      (uint8_t*) sample_expect_vector_s);
    test_ecdsa_verify((uint8_t*)  message2,  4, (uint8_t*) test_expect_vector_r,
                      (uint8_t*) test_expect_vector_s);
    ...
    printf("\r\n\r\n");
    printf("hosal crypto ecdsa finish \n");
