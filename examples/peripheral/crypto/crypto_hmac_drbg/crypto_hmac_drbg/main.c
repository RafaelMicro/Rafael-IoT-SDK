...
void hmac_test_vector1(void) {
    ...
    hosal_hmac_drbg_instantiate(&hmac_drbg_ctx, (uint8_t*)seed_material, sizeof(entropy_input) + sizeof(nonce));


    /*drbg reseed*/
    hosal_hmac_drbg_update(&hmac_drbg_ctx, (uint8_t *) entropy_input_reseed, sizeof(entropy_input_reseed));

    for (i = 0; i < NUMBER_OF_GENERATES; i++)
    {
        hosal_hmac_drbg_generate((uint8_t*)result, &hmac_drbg_ctx, 128, NULL, 0);
    }

    //hosal_hmac_drbg_dev_t hmac_drbg;

    //hmac_drbg.crypto_operation = HOSAL_HMAC_DRBG_HMAC;
    //hmac_drbg.seed_material = (uint8_t*)seed_material;
    //hmac_drbg.seed_material_length = sizeof(entropy_input) + sizeof(nonce);
    //hmac_drbg.in_ptr = NULL;
    //hmac_drbg.in_length = 0;
    //hmac_drbg.data_ptr = (uint8_t *) entropy_input_reseed;
    //hmac_drbg.data_len = sizeof(entropy_input_reseed);
    //hmac_drbg.out_ptr = (uint8_t*)result;
    //hmac_drbg.out_len = 128;
    //hosal_crypto_hmac_drbg_operation(&hmac_drbg);
...
void hmac_test_vector2(void) {
    ...
    //hmac_drbg.crypto_operation = HOSAL_HMAC_DRBG_HMAC;
    //hmac_drbg.seed_material = seed_material;
    //hmac_drbg.seed_material_length = sizeof(entropy_input) + sizeof(nonce);
    //hmac_drbg.in_ptr = (uint8_t*) entropy_input_reseed;
    //hmac_drbg.in_length = sizeof(entropy_input_reseed);
    //hmac_drbg.out_ptr = result;
    //hmac_drbg.out_len = 128;
    //hmac_drbg.data_len = 0;
...
void hmac_test_vector3(void) {
    ...
    uint8_t entropy_input_reseed[32] = {
    ...
    uint8_t additional_input_reseed[32] = {
    ...
    //hmac_drbg.out_ptr = result;
    //hmac_drbg.out_len = 128;
    //mac_drbg.data_len =32;
    //h/osal_crypto_hmac_drbg_operation(&hmac_drbg);
        ...
        /*drbg reseed*/
        hosal_hmac_drbg_update(&hmac_drbg_ctx, (uint8_t *) seed_material, sizeof(entropy_input_reseed) + sizeof(additional_input_reseed));

        for (i = 0; i < NUMBER_OF_GENERATES; i++)
        {
            hosal_hmac_drbg_generate((uint8_t*)result, &hmac_drbg_ctx, 128,  (uint8_t *)(additional_input[i]), 32);
        }
...
int main(void) {
    ...
    printf("Build Time:%s \r\n",__TIME__);
    printf("----------------------------------------------------------------\r\n");
    printf("Examples    : hosal crypto NIST hmac drbg demo \r\n");
    printf("----------------------------------------------------------------\r\n");
    ...
    printf("\r\n\r\n");
    printf("Examples    : hosal crypto NIST hmac drbg finish \r\n");
