...
#else
#define I2S_RD_BUFF       0x30002000
#define I2S_WR_BUFF       0x30004000
#define I2S_WR_DEBUG_BUFF 0x30007000
#endif
...
void init_i2s_fill_data_to_memory(void) {
    uint32_t idx;
...
void i2s_fill_data_to_memory(hosal_i2s_para_set_t* i2s_para)
{
    ...
    uint32_t i2s_rdma_read_ptr = 0;
    uint8_t loop;

    hosal_i2s_get_rdma_access_pos(&i2s_rdma_read_ptr);
    /* The same phase */
...
void i2s_data_memory_verification() {
    uint32_t i2s_wdma_write_ptr = 0;
    uint32_t error_cnt = 0;
    volatile uint8_t i = 0;

    i2s_get_wdma_access_pos(&i2s_wdma_write_ptr);
