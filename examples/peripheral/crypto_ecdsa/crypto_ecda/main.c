...
void  test_ecdsa_signature(uint8_t *msg, uint32_t msg_len, uint8_t *exp_r, uint8_t *exp_s) {
    ...
    if (( memcmp(seed_material, exp_r, ECDSA_P256_LENGTH) != 0) ||
        ...
        printf("OOPS, error  %c%c", '\r', '\n');
        while (1);
    }

    printf("Success for test vector  %c%c", '\r', '\n');
...
void test_ecdsa_verify(uint8_t  *msg, uint32_t  msg_length, uint8_t *vector_r, uint8_t *vector_s) {
    ...
    if (status != STATUS_SUCCESS) {
        printf("Oops, Error in ECDSA p256 verify  %c%c", '\r', '\n');
        while (1);
    }

    printf("ECDSA p256 verification success  %c%c", '\r', '\n');
    printf("%c%c", '\r', '\n');
        ...
        for (j = 0; j < 4096; j++) {
            ...
            for (i = j; i < 4096; i++) {
                ...
                if (status != STATUS_SUCCESS) {
                    ...
                    printf("error %ld %ld %ld   %lx %c", length, j,  i,  (uint32_t) (test_ptr), '\n');
