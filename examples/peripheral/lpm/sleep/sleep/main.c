 ...
 */
#if defined(CONFIG_RT584L)
void pwr_level4_int_callback(uint32_t pin, void* isr_param) {
...
void gpio_init(void) {
...
#if defined(CONFIG_RT584H) || defined(CONFIG_RT581) || defined(CONFIG_RT582) || defined(CONFIG_RT583)
    ...
    pin_cfg4.usr_cb = pwr_level4_int_callback;

    hosal_gpio_set_debounce_time(DEBOUNCE_SLOWCLOCKS_1024);
    hosal_gpio_debounce_enable(PWR_LEVEL4_KEY);
    hosal_pin_set_pullopt(PWR_LEVEL4_KEY, HOSAL_PULL_UP_100K);
    hosal_gpio_cfg_input(PWR_LEVEL4_KEY, pin_cfg4);
    hosal_gpio_int_enable(PWR_LEVEL4_KEY);
...
int main(void) {
    ...
    #else
    printf("press PWR_LEVEL4_KEY to enter power down mode\r\n");
    printf("press PWR_LEVEL1_KEY to enter deep sleep mode\r\n");
    printf("press PWR_LEVEL0_KEY to enter sleep mode \r\n");
    #endif
