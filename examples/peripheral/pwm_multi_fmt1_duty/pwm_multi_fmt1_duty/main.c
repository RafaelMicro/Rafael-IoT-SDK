...
int main(void) {
    ...
    printf("hosal pwm format 1 multi duty examples %s %s %c%c", __DATE__, __TIME__, '\r', '\n');
    ...
    pwm_dev.config.dma_smp_fmt = PWM_DMA_SMP_FMT_1;
    ...
    for (i = 0; i < 10; i++) {
        ...
        hosal_pwm_multi_fmt1_duty_ex(pwm_dev.config.id, &pwm_dev, i, duty);
        duty += 5;
    }
