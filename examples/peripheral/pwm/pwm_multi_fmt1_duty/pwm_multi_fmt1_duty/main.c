...
int main(void) {
    ...
    printf("Build Time:%s \r\n",__TIME__);
    printf("----------------------------------------------------------------\r\n");
    printf("Examples    : hosal pwm format 1 multi duty demo\r\n");
    printf("[PWM0 config]\r\n");
    ...
    pwm_dev.config.dma_smp_fmt = HOSAL_PWM_DMA_SMP_FMT_1;
    ...
    pwm_dev.config.count_end_val = 0; //using format 1 the count end value don't care
    ...
    for (i = 0; i < 10; i++) {
        ...
        hosal_pwm_multi_fmt1_duty(pwm_dev.config.id, &pwm_dev, i, duty);
        duty += 5;
    }
    ...
    printf("\r\n\r\n");
    printf("hosal pwm format 1 multi duty finish\r\n");
