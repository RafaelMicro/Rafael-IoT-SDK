...
{
    size_t actual_len;
    efd_get_env_blob("pin", (void *) get_pincode(), 17, &actual_len);
    if(actual_len == 0) {
        log_info("pincode not found, use default pincode");
        pincode_update();
    }
}
void pincode_update(void)
{
    efd_set_env_blob("pin", (void *) get_pincode(), 17);
}
...
void zigbee_app_toggle_lock_state(void) {
    uint8_t new_state;
    
    if(get_lock_state() == ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_LOCKED) {
        set_led_onoff(LED_RED, 1);
        new_state = ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_UNLOCKED;
        log_info("LOCK STATE: UNLOCKED");
    }
    else {
        set_led_onoff(LED_RED, 0);
        new_state = ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_LOCKED;
        log_info("LOCK STATE: LOCKED");
    }
    zigbee_zcl_set_attrubute(DOOR_LOCK_EP, ZB_ZCL_CLUSTER_ID_DOOR_LOCK, ZB_ZCL_CLUSTER_SERVER_ROLE,
            ZB_ZCL_ATTR_DOOR_LOCK_LOCK_STATE_ID, (uint8_t*) &new_state);
}
