...
void app_main_loop(void* parameters_ptr) {
    ...
    ZB_THREAD_SAFE(
        ZB_AF_REGISTER_DEVICE_CTX(&simple_desc_thermostat_ctx);
        register_app_cb(zigbee_app_thermostat_cb);
        for (int i = 0; i < simple_desc_thermostat_ctx.ep_count; i++) {
            ZB_AF_SET_ENDPOINT_HANDLER(
                simple_desc_thermostat_ctx.ep_desc_list[i]->ep_id,
