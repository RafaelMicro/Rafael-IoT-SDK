...
void zigbee_app_thermostat_cb(uint8_t param) {
    zb_zcl_device_callback_param_t *cb_param = ZB_BUF_GET_PARAM(param, zb_zcl_device_callback_param_t);
    if(cb_param->device_cb_id == ZB_ZCL_THERMOSTAT_VALUE_CB_ID) {
        int16_t heat_setpoint = cb_param->cb_param.thermostat_value_param.heat_setpoint;
        int16_t cool_setpoint = cb_param->cb_param.thermostat_value_param.cool_setpoint;
        log_info("heat setpoint %d", heat_setpoint);
        log_info("cool setpoint %d", cool_setpoint);
    }
}
