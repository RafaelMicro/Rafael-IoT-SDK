...
void app_main_loop(void* parameters_ptr) {
    ...
    ZB_THREAD_SAFE(
        ZB_AF_REGISTER_DEVICE_CTX(&simple_desc_wall_switch_ctx);
        for (int i = 0; i < simple_desc_wall_switch_ctx.ep_count; i++) {
            ZB_AF_SET_ENDPOINT_HANDLER(
                simple_desc_wall_switch_ctx.ep_desc_list[i]->ep_id,
        ...
        if (ulTaskNotifyTake(pdFALSE, portMAX_DELAY) != 0) {
            ...
            switch (sevent) {
                ...
                case ZB_APP_EVENT_TOGGLE: {
                    zigbee_app_send_toggle_command();
                } break;
                case ZB_APP_EVENT_LEVEL_UP: {
                    zigbee_app_send_level_up_command();
                } break;
                case ZB_APP_EVENT_LEVEL_DOWN: {
                    zigbee_app_send_level_down_command();
                } break;
                default: break;
            }
        }
