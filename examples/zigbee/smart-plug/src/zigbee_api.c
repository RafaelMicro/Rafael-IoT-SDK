...
void zigbee_app_toggle_onoff_state(void) {
    uint8_t new_state;
    
    if(get_on_off_status() == ZB_ZCL_ON_OFF_IS_OFF) {
        set_plug_status(1);
        new_state = ZB_ZCL_ON_OFF_IS_ON;
        log_info("PLUG STATE: ON");
    }
    else {
        set_plug_status(0);
        new_state = ZB_ZCL_ON_OFF_IS_OFF;
        log_info("PLUG STATE: OFF");
    }
    zigbee_zcl_set_attrubute(SMART_PLUG_EP, ZB_ZCL_CLUSTER_ID_ON_OFF, ZB_ZCL_CLUSTER_SERVER_ROLE,
            ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID, (uint8_t*) &new_state);
}
