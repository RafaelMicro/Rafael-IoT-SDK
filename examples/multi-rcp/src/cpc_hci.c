...
typedef struct {
    ...
    uint32_t hci_tx_done       : 4;
    uint32_t hci_event_trigger : 8;
    uint32_t hci_cmd_flag      : 1;
    uint32_t hci_cmd_comp_idx  : 8;
    uint32_t reserved          : 2;
...
static inline void cpc_hci_buffer_send(uint8_t* p_data, uint32_t data_len) {
    ...
    do {
        while (xStreamBufferSpacesAvailable(hci_mgr.xStreamBuffer) < data_len) {
            /* Wait */
...
static inline void handle_set_mac_address_cmd(uint8_t* p_cpc_buffer) {
    ...
    if (p_tmp_buf) {
        ...
        memcpy((p_tmp_buf + 2), &p_cpc_buffer[9], 6);
        memcpy((p_tmp_buf + 8), &p_cpc_buffer[9], 3);
        memcpy((p_tmp_buf + 13), &p_cpc_buffer[6], 3);
        p_tmp_buf[11] = 0xFF;
        p_tmp_buf[12] = 0xFF;
...
static void __cpc_hci_ep_proc(cpc_hci_event_t evt) {
        ...
        do {
            ...
            if (rval) {
                ...
                if (p_cpc_buffer[0] == 0x01 && p_cpc_buffer[1] == 0x0c
                    ...
                    hci_mgr.hci_cmd_flag = 1;
                    if (p_cpc_buffer[4] == 0x01) {
                        log_info("Scan Enable!");
                    } else {
                        log_info("Scan Disable!");
                    }
                }
    ...
    //     log_info("tx_done %d", hci_mgr.hci_tx_done);
    ...
    if (hci_mgr.hci_tx_done > 0) {
        ...
        if (xBytes2send == 0) {
            // MCU_ATOMIC_SECTION(hci_mgr.hci_event_trigger = 0;);
            // log_info("tri %d =0", hci_mgr.hci_event_trigger);
            return;
        }
...
void cpc_hci_init(void) {
    ...
    CPC_TASK_CREATE(__cpc_hci_task, "app-hci", 512, NULL,
