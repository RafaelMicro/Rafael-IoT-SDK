...
static void
__cmd_device_binding_information_request(zigbee_cmd_req_t* pt_cmd_req);
...
static void __cmd_channel_energy_scan_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_network_addr_table_update_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_get_address_by_group_idx_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_gateway_reset_request(zigbee_cmd_req_t* pt_cmd_req);
...
static void __cmd_gateway_panid_channel_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_gateway_install_code_set_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_gateway_install_code_remove_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_gateway_install_code_remove_all_request(zigbee_cmd_req_t* pt_cmd_req);
static void __cmd_gateway_standart_time_set(zigbee_cmd_req_t* pt_cmd_req);
static zigbee_cmd_func g_zb_cmd_func[] = {
    ...
    [ZIGBEE_CMD_DEVICE_BINDING_INFORMATION_REQUEST] =
        __cmd_device_binding_information_request,
    ...
    [ZIGBEE_CMD_CHANNEL_ENERGY_SCAN_REQUEST] = __cmd_channel_energy_scan_request,
    [ZIGBEE_CMD_NETWORK_ADDR_TABLE_UPDATE_REQUEST] = __cmd_network_addr_table_update_request,
    [ZIGBEE_CMD_GET_NETWORK_ADDR_VIA_GROUP_IDX_REQUEST] = __cmd_get_address_by_group_idx_request,
    [ZIGBEE_CMD_GATEWAY_RESET_REQUEST] = __cmd_gateway_reset_request,
    [ZIGBEE_CMD_GATEWAY_EXT_ADDRESS_REQUEST] =
        __cmd_gateway_ext_address_request,
    [ZIGBEE_CMD_GATEWAY_PERMITJOIN_STATUS_REQUEST] =
        __cmd_gateway_permitjoin_status_request,
    [ZIGBEE_CMD_GATEWAY_PANID_CHANNEL_REQUEST] =
        __cmd_gateway_panid_channel_request,
    [ZIGBEE_CMD_GATEWAY_INSTALL_CODE_SET_REQUEST] =
        __cmd_gateway_install_code_set_request,
    [ZIGBEE_CMD_GATEWAY_INSTALL_CODE_REMOVE_REQUEST] =
        __cmd_gateway_install_code_remove_request,
    [ZIGBEE_CMD_GATEWAY_INSTALL_CODE_REMOVE_ALL_REQUEST] =
        __cmd_gateway_install_code_remove_all_request,
    [ZIGBEE_CMD_GATEWAY_STANDARD_TIME_SET] = __cmd_gateway_standart_time_set,
};
...
static void _network_address_response(zb_uint8_t param) {
    ...
    zigbee_gw_cmd_send((ZIGBEE_CMD_NWK_ADDRESS_REQUEST | 0x8000), 0x0000, 0, 0,
                       nwk_addr_rsp_pd, sizeof(nwk_addr_rsp_pd));
}
...
static void _ieee_address_response(zb_uint8_t param) {
    ...
    zigbee_gw_cmd_send((ZIGBEE_CMD_IEEE_ADDRESS_REQUEST | 0x8000),
                       resp->nwk_addr, 0, 0, nwk_addr_rsp_pd,
                       sizeof(nwk_addr_rsp_pd));
...
static void _node_descriptor_response(zb_uint8_t param) {
    ...
    do {
        ...
        memcpy(&prsp_pd[3], (uint8_t*)&resp->node_desc,
               sizeof(zb_af_node_desc_t));
        zb_buf_free(param);
    } while (0);

    if (prsp_pd) {
        zigbee_gw_cmd_send((ZIGBEE_CMD_NODE_DESCRIPTOR_REQUEST | 0x8000),
                           resp->hdr.nwk_addr, 0, 0, prsp_pd,
                           sizeof(zb_af_node_desc_t) + 1 + 2);
        vPortFree(prsp_pd);
    } else {
        memcpy(&p_fail_pd[1], (uint8_t*)&resp->hdr.nwk_addr, 2);
        zigbee_gw_cmd_send((ZIGBEE_CMD_NODE_DESCRIPTOR_REQUEST | 0x8000),
                           resp->hdr.nwk_addr, 0, 0, p_fail_pd,
                           sizeof(p_fail_pd));
    }
}
...
static void _power_descriptor_response(zb_uint8_t param) {
    ...
    do {
        ...
        prsp_pd[0] = resp->hdr.status;
        memcpy(&prsp_pd[1], (uint8_t*)&resp->hdr.nwk_addr, 2);
        memcpy(&prsp_pd[3], (uint8_t*)&resp->power_desc,
               sizeof(zb_af_node_power_desc_t));
        zb_buf_free(param);
    } while (0);

    if (prsp_pd) {
        zigbee_gw_cmd_send((ZIGBEE_CMD_POWER_DESCRIPTOR_REQUEST | 0x8000),
                           resp->hdr.nwk_addr, 0, 0, prsp_pd,
                           sizeof(zb_af_node_power_desc_t) + 1 + 2);
        vPortFree(prsp_pd);
    } else {
        memcpy(&p_fail_pd[1], (uint8_t*)&resp->hdr.nwk_addr, 2);
        zigbee_gw_cmd_send((ZIGBEE_CMD_POWER_DESCRIPTOR_REQUEST | 0x8000),
...
static void _simple_descriptor_response(zb_uint8_t param) {
    ...
    do {
        ...
        total_cluster_count = resp->simple_desc.app_input_cluster_count
        ...
        pkt_len =
            ...
            + ((total_cluster_count)
               * sizeof(uint16_t)); // -4 Take into accout app_cluster_list
        ...
        prsp_pd[0] = resp->hdr.status;
        memcpy(&prsp_pd[1], (uint8_t*)&resp->hdr.nwk_addr, 2);
        prsp_pd[3] = sizeof(zb_af_simple_desc_1_1_t)
                     + ((total_cluster_count) * sizeof(uint16_t))
                     - (sizeof(uint16_t)
                        * 2); // -4 Take into accout app_cluster_list

        // log_info("descriptro length: %d\r\n", prsp_pd[3]);

        zb_copy_simple_desc((zb_af_simple_desc_1_1_t*)&prsp_pd[4],
                            &resp->simple_desc);
        zb_buf_free(param);
    } while (0);

    if (prsp_pd) {
        zigbee_gw_cmd_send((ZIGBEE_CMD_SIMPLE_DESCRIPTOR_REQUEST | 0x8000),
                           resp->hdr.nwk_addr, 0, 0, prsp_pd, pkt_len);
        vPortFree(prsp_pd);
    } else {
        memcpy(&p_fail_pd[1], (uint8_t*)&resp->hdr.nwk_addr, 2);
        zigbee_gw_cmd_send((ZIGBEE_CMD_SIMPLE_DESCRIPTOR_REQUEST | 0x8000),
...
static void _active_endpoint_response(zb_uint8_t param) {
    ...
    if (prsp_pd) {
        zigbee_gw_cmd_send((ZIGBEE_CMD_ACTIVE_ENDPOINT_REQUEST | 0x8000),
                           resp->nwk_addr, 0, 0, prsp_pd,
                           (resp->ep_count * sizeof(uint8_t)) + 1 + 2 + 1);
        vPortFree(prsp_pd);
    } else {
        ...
        zigbee_gw_cmd_send((ZIGBEE_CMD_ACTIVE_ENDPOINT_REQUEST | 0x8000),
                           resp->nwk_addr, 0, 0, p_fail_pd, sizeof(p_fail_pd));
    }
}
...
static void _bind_response(zb_uint8_t param) {
    ...
    zigbee_gw_cmd_send((ZIGBEE_CMD_BIND_REQUEST | 0x8000),
                       g_tsn_adrr_tbl[resp->tsn], 0, 0, &p_fail_pd,
                       sizeof(p_fail_pd));
}
...
static void _unbind_response(zb_uint8_t param) {
    ...
    zigbee_gw_cmd_send((ZIGBEE_CMD_UNBIND_REQUEST | 0x8000),
                       g_tsn_adrr_tbl[resp->tsn], 0, 0, &p_fail_pd,
                       sizeof(p_fail_pd));
}
...
static void _device_binding_information_response(zb_uint8_t param) {
    ...
    do {
        ...
        prsp_pd = pvPortMalloc(resp->binding_table_list_count
                                   * sizeof(zb_zdo_binding_table_record_t)
                               + sizeof(p_fail_pd));
        ...
        pt_binding_table =
            (zb_zdo_binding_table_record_t*)((zb_uint8_t*)resp
                                             + sizeof(zb_zdo_mgmt_bind_resp_t));

        prsp_pd[0] = resp->status;
        prsp_pd[1] = resp->binding_table_entries;
        prsp_pd[2] = resp->start_index;
        prsp_pd[3] = resp->binding_table_list_count;
        memcpy(&prsp_pd[4], (uint8_t*)pt_binding_table,
    ...
    if (prsp_pd) {
        zigbee_gw_cmd_send(
            (ZIGBEE_CMD_DEVICE_BINDING_INFORMATION_REQUEST | 0x8000),
            g_tsn_adrr_tbl[resp->tsn], 0, 0, prsp_pd,
            (resp->binding_table_list_count
                 * sizeof(zb_zdo_binding_table_record_t)
             + sizeof(p_fail_pd)));
        vPortFree(prsp_pd);
        ...
        zigbee_gw_cmd_send(
            (ZIGBEE_CMD_DEVICE_BINDING_INFORMATION_REQUEST | 0x8000),
            g_tsn_adrr_tbl[resp->tsn], 0, 0, p_fail_pd, sizeof(p_fail_pd));
    }
}
...
static void _device_leave_response(zb_uint8_t param) {
    ...
    zigbee_gw_cmd_send((ZIGBEE_CMD_DEVICE_LEAVE_REQUEST | 0x8000),
                       g_tsn_adrr_tbl[resp->tsn], 0, 0, &p_fail_pd,
                       sizeof(p_fail_pd));
}
...
static void __cmd_device_leave_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    do {
        ...
        if (pt_cmd_req->cmd_dst_addr == 0x0000) {
            req_params->dst_addr = zb_address_short_by_ieee(
                req_params->device_address);
            // log_info("target: %04x\r\n", req_params->dst_addr);
        } else {
            req_params->dst_addr = pt_cmd_req->cmd_dst_addr;
        }
        if (ZB_NWK_IS_ADDRESS_BROADCAST(req_params->dst_addr)) {
            tsn = 0xFF;
        } else {
...
static void __cmd_permit_join_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    do {
        pdata = (uint8_t*)pt_cmd_req->cmd_value;
        /* Enable permit join for coordinator itself */
        buf_zc = zb_buf_get_out();
        if (!buf_zc) {
            break;
        }
        req = ZB_BUF_GET_PARAM(buf_zc, zb_nlme_permit_joining_request_t);

        ZB_BZERO(req, sizeof(zb_nlme_permit_joining_request_t));
        req->permit_duration = pdata[0];

        zb_nlme_permit_joining_request(buf_zc);

        /* Enable permit join for other device */
        buf = zb_buf_get_out();
        if (!buf) {
            break;
        }
...
static void _network_update_notify(zb_uint8_t param) {
    ...
    do {
        ...
        if (resp->hdr.scanned_channels_list_count > 0) {
            memcpy(&prsp_pd[10], resp->energy_values,
                   resp->hdr.scanned_channels_list_count);
        }
    ...
    if (prsp_pd) {
        zigbee_gw_cmd_send((ZIGBEE_CMD_NETWORK_UPDATE_REQUEST | 0x8000),
                           g_tsn_adrr_tbl[resp->hdr.tsn], 0, 0, prsp_pd,
                           pkt_len);
    } else {
        zigbee_gw_cmd_send((ZIGBEE_CMD_NETWORK_UPDATE_REQUEST | 0x8000),
                           g_tsn_adrr_tbl[resp->hdr.tsn], 0, 0, &p_fail_pd,
                           sizeof(p_fail_pd));
    }
}
...
static void __cmd_network_start_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    zigbee_app_nwk_start(channel, 100, pan_id, reset);

    zigbee_gw_cmd_send((ZIGBEE_CMD_NETWORK_START_REQUEST | 0x8000), 0x0000, 0,
                       0, &status, 1);
}
static void __cmd_channel_energy_scan_request(zigbee_cmd_req_t* pt_cmd_req) {
...
static void __cmd_get_address_by_group_idx_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    zigbee_app_get_address_by_group_idx(group_idx);
}
static void __cmd_gateway_reset_request(zigbee_cmd_req_t* pt_cmd_req) {
    uint8_t magic_number;
    uint8_t status;

    magic_number = pt_cmd_req->cmd_value[0];

    if(magic_number == 0x88) {
        status = 0;
        zigbee_gw_cmd_send((ZIGBEE_CMD_GATEWAY_RESET_REQUEST | 0x8000), 0x0000, 0,
                        0, &status, 1);
        vTaskDelay(300);
        sys_software_reset();
    } else {
...
static void __cmd_gateway_ext_address_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    log_info("ext addr: %02x %02x %02x %02x %02x %02x %02x %02x", extAddr[7],
             extAddr[6], extAddr[5], extAddr[4], extAddr[3], extAddr[2],
             extAddr[1], extAddr[0]);

    temp[0] = status;
    memcpy(&temp[1], extAddr, 8);

    zigbee_gw_cmd_send((ZIGBEE_CMD_GATEWAY_EXT_ADDRESS_REQUEST | 0x8000),
                       0x0000, 0, 0, temp, 9);
}

static void
__cmd_gateway_permitjoin_status_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    log_info("association permit: %s",
             status.association_permit ? "true" : "false");
    ...
    zigbee_gw_cmd_send((ZIGBEE_CMD_GATEWAY_PERMITJOIN_STATUS_REQUEST | 0x8000),
                       0x0000, 0, 0, (void*)&status,
                       sizeof(permit_join_status_t));
}
...
static void __cmd_gateway_panid_channel_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    temp[0] = status;
    panid = ZB_PIBCACHE_PAN_ID();
    memcpy(&temp[1], &panid, 2);
    log_info("PAN_ID: 0x%04x", ZB_PIBCACHE_PAN_ID());
    temp[3] = ZB_PIBCACHE_CURRENT_CHANNEL();
    log_info("Current channel: %d(0x%02x)", temp[3], temp[3]);

    zigbee_gw_cmd_send((ZIGBEE_CMD_GATEWAY_PANID_CHANNEL_REQUEST | 0x8000),
                       0x0000, 0, 0, temp, 4);
}

static void _install_code_set_response(zb_ret_t ret) {
    uint8_t status;

    status = ret;

    zigbee_gw_cmd_send((ZIGBEE_CMD_GATEWAY_INSTALL_CODE_SET_REQUEST | 0x8000),
                       0x0000, 0, 0, &status, 1);
}

static void _install_code_remove_response(zb_uint8_t param) {
    uint8_t status;
    zb_secur_ic_remove_resp_t* resp_param = NULL;

    resp_param = ZB_BUF_GET_PARAM(param, zb_secur_ic_remove_resp_t);
    status = resp_param->status;

    zigbee_gw_cmd_send(
        (ZIGBEE_CMD_GATEWAY_INSTALL_CODE_REMOVE_REQUEST | 0x8000), 0x0000, 0, 0,
        &status, 1);
    zb_buf_free(param);
}

static void _install_code_remove_all_response(zb_uint8_t param) {
    uint8_t status;
    ...
    zigbee_gw_cmd_send(
        (ZIGBEE_CMD_GATEWAY_INSTALL_CODE_REMOVE_ALL_REQUEST | 0x8000), 0x0000,
        0, 0, &status, 1);
    zb_buf_free(param);
}

static void
__cmd_gateway_install_code_set_request(zigbee_cmd_req_t* pt_cmd_req) {
    install_code_add_t* ic;

    ic = (install_code_add_t*)pt_cmd_req->cmd_value;
    zb_secur_ic_add(ic->ieeeAddr, ic->ic_type, ic->ic,
                    _install_code_set_response);
}

static void
__cmd_gateway_install_code_remove_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    do {
        ...
        req_param = ZB_BUF_GET_PARAM(buf, zb_secur_ic_remove_req_t);
        ZB_IEEE_ADDR_COPY(req_param->device_address, pt_cmd_req->cmd_value);
        req_param->response_cb = _install_code_remove_response;
        zb_secur_ic_remove_req(buf);
    } while (0);
}

static void
__cmd_gateway_install_code_remove_all_request(zigbee_cmd_req_t* pt_cmd_req) {
    ...
    do {
        ...
        req_param = ZB_BUF_GET_PARAM(buf, zb_secur_ic_remove_all_req_t);
        req_param->response_cb = _install_code_remove_all_response;
...
static void __cmd_gateway_standart_time_set(zigbee_cmd_req_t* pt_cmd_req) {
    uint8_t* pdata;
    uint32_t standart_time;
    do {
        pdata = (uint8_t*)pt_cmd_req->cmd_value;
        standart_time = *(uint32_t*)pdata;
        log_info("Set gateway standard time 0x%x\n", standart_time);
        set_gw_time(standart_time, true);
    } while (0);
}
