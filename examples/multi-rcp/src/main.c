...
static void wdt_kick_loop(void* parameters_ptr);
...
static void init_wdt_init(void) {
    ...
    wdt_cfg_ticks.wdt_ticks = 10000 * 1000;
    wdt_cfg_ticks.int_ticks = 1000 * 1000;
...
int main(void) {
    ...
    zbStart();
#endif
    xTaskCreate(wdt_kick_loop, "wdt", 256, NULL,
                            1, &wdt_task_handle);
    vTaskStartScheduler();
    while(1) {;}
}
void wdt_kick_loop(void* parameters_ptr) {
    while(1) {
        if (!s_reboot_flag)
            wdt_kick();
        vTaskDelay(1000);
