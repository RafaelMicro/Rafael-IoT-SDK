...
#define ZB_OTA_UNCOMPRESS_ADDR FOTA_UPDATE_BUFFER_FW_ADDRESS_2MB_UNCOMPRESS
#elif defined(CONFIG_RT584HA4)
#define ZB_OTA_UNCOMPRESS_ADDR FOTA_UPDATE_BUFFER_FW_ADDRESS_4MB_UNCOMPRESS
#else
#define ZB_OTA_UNCOMPRESS_ADDR FOTA_UPDATE_BUFFER_FW_ADDRESS_1MB_UNCOMPRESS
#endif
//=============================================================================
//                Private ENUM
//=============================================================================
...
static void _check_ota_file_valid(ota_file_info_t *file_info)
{
    ...
    tmp[0] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 0x34);
    tmp[1] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 0x35);
    tmp[2] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 0x36);
    tmp[3] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 0x37);
    memcpy(&file_info->image_size, tmp, 4);
    while (flash_check_busy()) {} ;

    if(file_info->image_size<SIZE_OF_FOTA_BANK_UNCOMPRESS)  //464*1024-4=0x73FFC
    {  
        crc32 = crc32checksum(ZB_OTA_UNCOMPRESS_ADDR, file_info->image_size);
        gt_img_info.image_size = file_info->image_size;
        log_info("image_size: 0x%08X", file_info->image_size);
        log_info("crc32: 0x%08X", crc32);

        tmp[0] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + file_info->image_size + 0);
        tmp[1] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + file_info->image_size + 1);
        tmp[2] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + file_info->image_size + 2);
        tmp[3] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + file_info->image_size + 3);
        ...
        if(crc32 == crcread) 
        {
            ...
            tmp[0] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 10);
            tmp[1] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 11);
            ...
            tmp[0] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 12);
            tmp[1] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 13);
            ...
            tmp[0] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 14);
            tmp[1] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 15);
            tmp[2] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 16);
            tmp[3] = flash_read_byte(ZB_OTA_UNCOMPRESS_ADDR + 17);
...
void _gw_ota_cmd_handle(uint32_t cmd_id, uint8_t *pBuf)
{
    ...
    static uint32_t flash_addr = ZB_OTA_UNCOMPRESS_ADDR, tmp_len = 0;
    ...
    if (cmd_id == GW_CMD_OTA_UPLOAD_START_REQUEST)
    {
        recv_cnt = 0;
        tmp_len = 0;
        flash_addr = ZB_OTA_UNCOMPRESS_ADDR;
        for (i = 0; i < 0x74; i++)
        {
            // Page erase (4096 bytes)
            while (flash_check_busy());
            flash_erase(FLASH_ERASE_SECTOR, ZB_OTA_UNCOMPRESS_ADDR + (0x1000 * i));
        }
    ...
    else if (cmd_id == GW_CMD_OTA_BLOCK_REQUEST)
    {
        ...
        if (gp_ota_imgae_cache)
        {
            ...
            if (upg_data->cur_pkt == (gt_img_info.total_pkt - 1))
            {
                ...
                flush_cache();
                crc32 = crc32checksum(ZB_OTA_UNCOMPRESS_ADDR, gt_img_info.image_size);

                //if (((0x0007C000 + gt_img_info.image_size) % 0x1000) > 0)
                {
                    poffset = (ZB_OTA_UNCOMPRESS_ADDR + gt_img_info.image_size) - ((ZB_OTA_UNCOMPRESS_ADDR + gt_img_info.image_size) % 0x1000);
                }
                ...
                memcpy(&gp_ota_imgae_cache[((ZB_OTA_UNCOMPRESS_ADDR + gt_img_info.image_size) % 0x1000)], &crc32, 4);
    ...
    else if (cmd_id == GW_CMD_OTA_FILE_INSERT_REQUEST)
    {
        ...
        if(file_info.image_found) {
            ...
            _ota_file_insert(ZB_OTA_UNCOMPRESS_ADDR, gt_img_info.image_size + 4, gt_img_info.file_version, gt_img_info.image_type, gt_img_info.manufacturer_code);
