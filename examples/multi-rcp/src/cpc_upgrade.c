...
#if defined(CONFIG_RT583) || defined(CONFIG_RT584H) || defined(CONFIG_RT584L)
#define CPC_UPG_UNCOMPRESS_ADDR FOTA_UPDATE_BUFFER_FW_ADDRESS_2MB_UNCOMPRESS
#elif defined(CONFIG_RT584HA4)
#define CPC_UPG_UNCOMPRESS_ADDR FOTA_UPDATE_BUFFER_FW_ADDRESS_4MB_UNCOMPRESS
#else
#define CPC_UPG_UNCOMPRESS_ADDR FOTA_UPDATE_BUFFER_FW_ADDRESS_1MB_UNCOMPRESS
#endif
...
SENUM_GENERIC(cpc_upg_command_t, uint32_t){
    ...
    CPC_ZB_GW_OTA_CMD_ERASE = 0xE1000000,
    CPC_ZB_GW_OTA_CMD_WRITE = 0xE1000001,
    CPC_ZB_GW_OTA_CMD_COMPLETE = 0xE1000002,
    CPC_ZB_GW_OTA_CMD_END = 0xE1000003,

    CPC_ZB_GW_OTA_CMD_SEND_ERASE = 0xE1008000,
    CPC_ZB_GW_OTA_CMD_SEND_WRITE = 0xE1008001,
    CPC_ZB_GW_OTA_CMD_SEND_COMPLETE = 0xE1008002,
    CPC_ZB_GW_OTA_CMD_SEND_END = 0xE1008003,
};
...
static g_cpc_upg_t upg_mgr = {
    .fota = {0},
    .cache_idx = 0,
    .flash_addr = CPC_UPG_UNCOMPRESS_ADDR,
};
...
static inline void zbgw_cmd_erase(uint8_t* pBuf) {
    ...
    //allocate for backup dataAdd commentMore actions
...
static inline void zbgw_cmd_write(uint8_t* pBuf) {
    ...
    if (upg_mgr.cache) {
        if (recv_cnt + 0x100 >= 0x1000) {
            ...
            log_info("erase addr 0x%08X", start_address+flash_addr);
            __cpc_flash_erase(FLASH_ERASE_SECTOR, start_address+flash_addr);
            log_info("write addr 0x%08X", start_address+flash_addr);

            // Write data in 256-byte chunks
            for (uint32_t i = 0; i < 0x10; i++) {
                __cpc_flash_write(start_address+flash_addr,
                                  (uint32_t)&upg_mgr.cache[i * 0x100]);
                //portYIELD();
                flash_addr += 0x100;
            }
...
static inline void upgrade_cmd_erase() {
    CPC_UPG_MALLOC_COND(!upg_mgr.cache, upg_mgr.cache, 0x1000);
    upg_mgr.cache_idx = 0;
    upg_mgr.flash_addr = CPC_UPG_UNCOMPRESS_ADDR;
...
static inline void upgrade_cmd_complete() {
    ...
    upg_mgr.fota.fotabank_startaddr = CPC_UPG_UNCOMPRESS_ADDR;
...
static inline void upgrade_cmd_ota_erase() {
    CPC_UPG_MALLOC_COND(!upg_mgr.cache, upg_mgr.cache, 0x1000);
    upg_mgr.cache_idx = 0;
    upg_mgr.flash_addr = CPC_UPG_UNCOMPRESS_ADDR;
    upgrade_cmd_send(CPC_ZB_GW_OTA_CMD_SEND_ERASE, 0, 0, 0, 0, 4);
}

static inline void upgrade_cmd_ota_write(upg_img_info_t* data) {
    uint32_t crc32 = 0;
    uint8_t* crcptr = (uint8_t*)&crc32;

    if (data->cur_pkt == 0) {
        memcpy((uint8_t*)&upg_mgr.image, data, sizeof(upg_mgr.image));
        upg_mgr.image.image_type = *(uint16_t*)(data->pkt + 0x0C);
        upg_mgr.image.manufacturer_code = *(uint16_t*)(data->pkt + 0x0A);
        upg_mgr.image.file_version = *(uint32_t*)(data->pkt + 0x0E);
        log_info("File Type: 0x%X", upg_mgr.image.image_type);
        log_info("Manufacturer Code: 0x%X", upg_mgr.image.manufacturer_code);
        log_info("File Version: 0x%X", upg_mgr.image.file_version);
        log_info("File Size: 0x%X", upg_mgr.image.image_size);
    ...
    if (data->cur_pkt == (upg_mgr.image.total_pkt - 1)) {
        uint32_t last_cache_index = upg_mgr.cache_idx;

        if (last_cache_index != 0) {
            WRITE_INAGE_COND(data->cur_pkt == (upg_mgr.image.total_pkt - 1));
        }

        crc32 = crc32checksum(CPC_UPG_UNCOMPRESS_ADDR, upg_mgr.image.image_size);

        if (last_cache_index == 0) {
            memset(upg_mgr.cache, 0xFF, 0x1000);
            memcpy(&upg_mgr.cache[last_cache_index], &crc32, 4);
            WRITE_INAGE_COND(1);
        }

        if (last_cache_index + 4 <= 0x1000 && last_cache_index != 0) {
            //add the image CRC32 to the flash, flash the last section address again
            upg_mgr.flash_addr = upg_mgr.flash_addr - 0x1000;
            memcpy(&upg_mgr.cache[last_cache_index], &crc32, 4);
            WRITE_INAGE_COND(1);
        } else if (last_cache_index + 4 > 0x1000) {
            upg_mgr.flash_addr = upg_mgr.flash_addr - 0x1000;
            uint8_t remain_space = 0x1000 - last_cache_index;
            memcpy(&upg_mgr.cache[last_cache_index], crcptr, remain_space);
            WRITE_INAGE_COND(1);

            memset(upg_mgr.cache, 0xFF, 0x1000);
            memcpy(&upg_mgr.cache[0], crcptr + remain_space, 4 - remain_space);
            WRITE_INAGE_COND(1);
        }

        uint8_t data[12];
        upg_mgr.image.image_size += 4;
        memcpy(&data[0], &upg_mgr.image.manufacturer_code, 2);
        memcpy(&data[2], &upg_mgr.image.image_type, 2);
        memcpy(&data[4], &upg_mgr.image.file_version, 4);
        memcpy(&data[8], &upg_mgr.image.image_size, 4);
        log_info("2File Type: 0x%X", upg_mgr.image.image_type);
        log_info("2Manufacturer Code: 0x%X", upg_mgr.image.manufacturer_code);
        log_info("2File Version: 0x%X", upg_mgr.image.file_version);
        log_info("2File Size: 0x%X", upg_mgr.image.image_size);
    ...
    // vTaskDelay(1);
    // ***************
    upgrade_cmd_send(CPC_ZB_GW_OTA_CMD_SEND_WRITE, 0, 0, 0,
                     (uint8_t*)&data->cur_pkt, 4);
}

static inline void upgrade_cmd_ota_complete() {
    memset(&upg_mgr.fota, 0xFF, sizeof(upg_mgr.fota));
    upg_mgr.fota.fotabank_ready = FOTA_IMAGE_READY;
    upg_mgr.fota.fotabank_startaddr = CPC_UPG_UNCOMPRESS_ADDR;
    upg_mgr.fota.target_startaddr = APP_START_ADDRESS;
    ...
    upgrade_cmd_send(CPC_ZB_GW_OTA_CMD_SEND_COMPLETE, 0, 0, 0, 0, 4);
    //upg_mgr.state = 1;
}
...
//  |     Reserved    (16K)    |
//  +--------------------------+-------> 0x000F_4000
static void upgrade_cmd_ota_handle(uint32_t cmd_id, uint8_t* pBuf) {
    switch (cmd_id) {
        case CPC_ZB_GW_OTA_CMD_ERASE: return upgrade_cmd_ota_erase();
        case CPC_ZB_GW_OTA_CMD_WRITE:
            return upgrade_cmd_ota_write((upg_img_info_t*)pBuf);
        case CPC_ZB_GW_OTA_CMD_COMPLETE: return upgrade_cmd_ota_complete();
        default: return;
    }
}
...
static void upgrade_cmd_proc(uint8_t* pBuf, uint32_t len) {
    ...
    else if ((cmd_index >= CPC_ZB_GW_OTA_CMD_ERASE)
             && (cmd_index < CPC_ZB_GW_OTA_CMD_END))
        upgrade_cmd_ota_handle(cmd_index, pt_pd->parameter);
#endif
}

static void __cpc_upg_ep_proc(cpc_upg_event_t evt) {
    if (CPC_UPG_EVENT_CPC_ERROR & evt) {
        if (cpc_get_endpoint_state(&upg_mgr.ep) == CPC_STATE_ERROR_FAULT)
            cpc_system_reset(0);
...
void cpc_upgrade_init(void) {
    ...
    CPC_TASK_CREATE(upgrade_cpc_task, "app-upgrade", 512, NULL,
                    E_TASK_PRIORITY_LOWEST, &upg_mgr.task);
}
