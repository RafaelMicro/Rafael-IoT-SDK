...
static uint16_t appUdpPort = CONFIG_APP_THREAD_UDP_LISTEN_PORT;
...
void app_udpSend(otInstance* instance, uint8_t* p, uint16_t len) {
    ...
    while (otThreadGetNextNeighborInfo(instance, &nbrIter, &neighborInfo)
           == OT_ERROR_NONE) {
        ...
        if (OT_ERROR_NONE == otMessageAppend(pmsg, p, len)
            ...
            if (OT_ERROR_NONE
                == otUdpSendDatagram(instance, pmsg, &messageInfo)) {
