...
extern otError otLinkBlackListAddPanId(otInstance *aInstance, const uint16_t aPanId);
        ...
        for (i = 0; i < NWK_MGM_CHILD_REQ_TATLE_SIZE; i++)
        {
                    ...
                    memcmp(nwk_mgm_child_reg_table[i].extaddr, extaddr, OT_EXT_ADDRESS_SIZE) == 0)
            {
                nwk_mgm_printf("updata %02X%02X%02X%02X%02X%02X%02X%02X \n", 
                    nwk_mgm_child_reg_table[i].extaddr[0], nwk_mgm_child_reg_table[i].extaddr[1],
                    nwk_mgm_child_reg_table[i].extaddr[2], nwk_mgm_child_reg_table[i].extaddr[3],
                    nwk_mgm_child_reg_table[i].extaddr[4], nwk_mgm_child_reg_table[i].extaddr[5],
                    nwk_mgm_child_reg_table[i].extaddr[6], nwk_mgm_child_reg_table[i].extaddr[7]);
...
static int _cli_cmd_nwk(int argc, char **argv, cb_shell_out_t log_out, void *pExtra)
{
